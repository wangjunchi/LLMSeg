...
class ReferSegDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        for ds in self.refer_seg_ds_list:
            ...
            self.coco2014_sam_helper = coco2014_sam_mask_helper 
            self.saiapr_tc12_sam_helper = saiapr_sam_mask_helper

            assert self.coco2014_sam_helper is not None and self.saiapr_tc12_sam_helper is not None
    ...
    def __getitem__(self, idx):
        ...
        # get sam segs
        if ds == "refclef":
            image_name_list = image_info["file_name"].split("/")[-3:]
            image_name = os.path.join(*image_name_list)
            segs_dict = self.saiapr_tc12_sam_helper.extract_sam_segs(image_name)
        else:
            segs_dict = self.coco2014_sam_helper.extract_sam_segs(image_info["file_name"].split("/")[-1])
        ...
        segs_square = segs_square.to(torch.float32)
                ...
                if -1 in ann_id:
                    ...
                    for ann_id_i in ann_id:
                        ...
                        if len(ann["segmentation"]) == 0:
                            ...
                            m = m.astype(bool)  # convert to bool
