...
from .utils import ANSWER_LIST, SHORT_QUESTION_LIST, compute_all_iou, compute_all_iop
...
def init_ade20k(base_image_dir):
    with open(os.path.join(PROJECT_DIR, "utils/ade20k_classes.json"), "r") as f:
...
def init_cocostuff(base_image_dir):
    cocostuff_classes = []
    with open(os.path.join(PROJECT_DIR, "utils/cocostuff_classes.txt"), "r") as f:
...
class SemSegDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        # check if sam mask helper is provided for each dataset
        if "ade20k" in self.sem_seg_datas:
            assert ade20k_sam_mask_helper is not None
            self.ade20k_sam_mask_helper = ade20k_sam_mask_helper
        
        if "mapillary" in self.sem_seg_datas:
            assert mapillary_sam_mask_helper is not None
            self.mapillary_sam_mask_helper = mapillary_sam_mask_helper
        
        if "cocostuff" in self.sem_seg_datas or "pacolvis" in self.sem_seg_datas:
            assert coco2017_sam_mask_helper is not None
            self.coco2017_sam_mask_helper = coco2017_sam_mask_helper

        if "pascal_part" in self.sem_seg_datas:
            assert voc2010_sam_mask_helper is not None
    ...
    def __getitem__(self, idx):
        ...
        # read sam segs
        if ds == "ade20k":
            segs_dict = self.ade20k_sam_mask_helper.extract_sam_segs(
                                        os.path.basename(image_path))
            
        elif ds == "mapillary":
            segs_dict = self.mapillary_sam_mask_helper.extract_sam_segs(
                                        os.path.basename(image_path))
        
        elif ds == "cocostuff" or ds == "paco_lvis":
            segs_dict = self.coco2017_sam_mask_helper.extract_sam_segs(
                                        os.path.basename(image_path))

        elif ds == "pascal_part":
            segs_dict = self.voc2010_sam_mask_helper.extract_sam_segs(
                                        os.path.basename(image_path))
        ...
        if ds in ["paco_lvis", "pascal_part"]:
            ...
            sampled_ious = [
                compute_all_iou(segs_origin, mask.astype(np.uint8)) for mask in masks
            ]
            sampled_iops = [
                compute_all_iop(segs_origin, mask.astype(np.uint8)) for mask in masks
            ]
            ...
            sampled_ious = [
                compute_all_iou(segs_origin, mask.numpy().astype(np.uint8)) for mask in masks
            ]
            sampled_iops = [
                compute_all_iop(segs_origin, mask.numpy().astype(np.uint8)) for mask in masks
            ]
         
            masks = torch.stack(masks, dim=0)
        
        ious = torch.from_numpy(np.stack(sampled_ious, axis=0))
        iops = torch.from_numpy(np.stack(sampled_iops, axis=0))
        # convert segs_square to tensor and resize to 64x64
        segs_square = torch.from_numpy(segs_square).permute(2, 0, 1).contiguous() # (K, H, W)
        segs = F.interpolate(segs_square.unsqueeze(0), size=(256, 256), mode="bilinear", align_corners=False) # (1, K, 64, 64)
