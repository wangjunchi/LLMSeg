...
from utils.dataset import HybridDataset, collate_fn, ValDataSet_ReasonSeg, collate_fn_new
        ...
        if any(
            [
                ...
                for x in ["lm_head", "embed_tokens", "text_hidden_fcs",
                          "lisa_mask_self_attn", "lisa_norm1", "lisa_cross_attn_mask_to_text", "lisa_norm2", "lisa_mlp",
                          "lisa_iou_head"]
            ]
        ):
...
def init_training_dataset(args, tokenizer):
    ...
    # init SAM mask helper
    sam_mask_dir = "/cluster/home/leikel/junchi/processed_data/"
    
    # # coco2017_sam_mask_helper = SAM_Mask_Reader(
    # #                         os.path.join(sam_mask_dir, "coco2017", "masks.json")) 
    # coco2014_sam_mask_helper = SAM_Mask_Reader(
    #                         os.path.join(sam_mask_dir, "coco2014", "masks.json"))
    # # voc2010_sam_mask_helper = SAM_Mask_Reader(
    # #                         os.path.join(sam_mask_dir, "voc2010", "masks.json"))
    # # ade20k_sam_mask_helper = SAM_Mask_Reader(
    # #                         os.path.join(sam_mask_dir, "ade20k", "masks.json"))
    # # mapillary_sam_mask_helper = SAM_Mask_Reader(
    # #                         os.path.join(sam_mask_dir, "mapillary", "masks.json"))
    # saiapr_sam_mask_helper = SAM_Mask_Reader(
    #                         os.path.join(sam_mask_dir, "saiapr", "masks.json"))
    # reasonseg_train_sam_mask_helper = SAM_Mask_Reader(
    #                         os.path.join(sam_mask_dir, "reason_seg", "ReasonSeg", "train", "masks.json"))
    # reasonseg_val_sam_mask_helper = SAM_Mask_Reader(
    ...
    train_dataset = HybridDataset(
        ...
        * world_size,
        ...
        # coco2014_sam_mask_helper=coco2014_sam_mask_helper,
        # coco2017_sam_mask_helper=coco2017_sam_mask_helper,
        # voc2010_sam_mask_helper=voc2010_sam_mask_helper,
        # ade20k_sam_mask_helper=ade20k_sam_mask_helper,
        # mapillary_sam_mask_helper=mapillary_sam_mask_helper,
        # saiapr_sam_mask_helper=saiapr_sam_mask_helper,
        # reasonseg_train_sam_mask_helper=reasonseg_train_sam_mask_helper,
        # reasonseg_val_sam_mask_helper=reasonseg_val_sam_mask_helper,
    )
...
def init_validation_dataset(args, tokenizer):
    ...
    val_dataset = ValDataSet_ReasonSeg(
        args.dataset_dir,
        tokenizer,
        args.vision_tower,
        args.image_size,
...
def main(args):
    ...
    if args.eval_only:
        ...
        giou, ciou = validate_threshold(val_loader, model_engine, 0, writer, args)
...
def validate_threshold(val_loader, model_engine, epoch, writer, args):
    print("start validating ###############################")
    intersection_meter = AverageMeter("Intersec", ":6.3f", Summary.SUM)
    union_meter = AverageMeter("Union", ":6.3f", Summary.SUM)
    acc_iou_meter = AverageMeter("gIoU", ":6.3f", Summary.SUM)
    ...
    for input_dict in tqdm.tqdm(val_loader):
        ...
        # pred_seg = sam_segs[:, :, max_idx] # (H, W)
        # pred_seg = torch.from_numpy(pred_seg).unsqueeze(0) # (1, H, W)
        pred_segs = torch.zeros_like(sam_segs[:, :, 0])
        for i in max_ids:
            pred_segs += sam_segs[:, :, i]
        pred_segs = pred_segs > 0
        pred_segs = pred_segs.unsqueeze(0).to(dtype = gt_mask.dtype)
